const takeElement = document.getElementById("takeElement") as HTMLDivElement;
const gridContainer = document.getElementById(
    "gridContainer"
) as HTMLDivElement;
const savePositionContainer = document.getElementById(
...
takeElement.addEventListener("pointerdown", (event) => {
    ...
    const getPositionCreatedElement = (coordinate: number): string => {
    ...
    const onPointerUpHandler = (event: PointerEvent) => {
        ...
        const isPointerInContainer = (container: HTMLDivElement): boolean => {
            return (
                event.pageY >=
                    window.pageYOffset +
